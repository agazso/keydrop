import { hexToString } from '../src/string';

it('should convert with prefix', () => {
    const input = '0x7b7d';
    const result = hexToString(input);
    expect(result).toEqual('{}');
});

it('should convert without prefix', () => {
    const input = '7b7d';
    const result = hexToString(input);
    expect(result).toEqual('{}');
});

it('should convert with real data', () => {
    const input = '';
    const result = hexToString(input);
    expect(result).toEqual('{\"type\":\"ping\",\"publicKey\":\"0x041b0276232dc0ff24999da23f267a5ef6ab6e0aa4118afa3ea3881ceb01f64781fbb7a8211bc0a21d5c30976c50be5536c461884b24e83b5ad662a8fafa31ae84\"}');
});
